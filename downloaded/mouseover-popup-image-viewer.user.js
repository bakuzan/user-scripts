...
function loadHosts() {
	var hosts = [
		...
		{r:/facebook\.com\/(photo\.php|[^\/]+\/photos\/)/, s:function(m, node) { if(node.id == 'fbPhotoImage') return false; if(/gradient\.png$/.test(m.input)) return ''; return m.input.replace('www.facebook.com', 'mbasic.facebook.com'); }, q:'div + span > a:first-child', rect:'#fbProfileCover'},
		...
		{r:/(hosturimage\.com|imageontime\.org|imggoo\.com|imgwel\.com|imageboom\.net|imageon\.org|img4ever\.net|imgcandy\.net|imgdevil\.com|imgrun\.net|imgult\.com|img\.yt|picspornfree\.me|pixliv\.com|pixxx\.me|uplimg\.com|xxxscreens\.com|xxxupload\.org)\/img-|imgbb\.net\/v-/, s:'/\\/(v-[0-9a-f]+)_.+//$1/', q:['img.centred_resized, #image', 'img[src*="/upload/big/"]'], xhr:true, post:'imgContinue=Continue%20to%20image%20...%20'},
		...
		{r:/pimpandhost\.com\/(image|guest)\//, s:'/small|medium/original/', q:'#image'},
		...
		{r:/postima?ge?\.org\/image\/\w+/, q:'center img'},
		...
		{r:/((awsmpic|damimage|imagedecode|imghit|imgtrial|ocaload|swoopic)\.com|(imgproof|imgserve|imgget)\.net|(dragimage|gogoimage|imgspot|imgstudio|madimage)\.org|imgs\.it|image\.re)\/img-/, q:'img.centred_resized, img.centred', xhr:true},
		...
		{r:/twitter\.com\/.+\/status\/.+\/photo\//, q:'.OldMedia img, .media img, video.animated-gif, .AdaptiveMedia-singlePhoto img', follow:function(url) { return !/\.mp4$/.test(url); }},
